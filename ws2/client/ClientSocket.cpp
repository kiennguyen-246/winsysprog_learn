...
HRESULT ClientSocket::sendMsg(std::wstring wsMsg) {
  int iResult = 0;
  iResult = send(connectSocket, (LPSTR)wsMsg.c_str(),
                 wsMsg.length() * sizeof(WCHAR), 0);
  if (iResult == SOCKET_ERROR) {
    HRESULT hrLastError = WSAGetLastError();
    wprintf(L"Send message failed 0x%08x\n", hrLastError);
    return hrLastError;
  } else {
    wprintf(L"Message sent: %ws", wsMsg.c_str());
  }
}